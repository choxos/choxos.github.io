...
| Item | Details |
|---|---|
| Protocol version | 3.0, draft for review (round 2) |
...
| Status | A pilot started on 19 September 2026 (Section 2.12). Confirmatory data collection starts on the day after this protocol is finalized; that date is recorded here and in the software as the study start, and reports made before it never enter a model. |
...
| Use of AI tools | AI tools helped write the study software and revise this protocol. Two AI models review the protocol; their reports and the responses are published with it (Section 3.4). |
...
Automatic collection: collection is automatic wherever a data source allows it. Ultrahuman grants access to its Partner API on request; with that access, an hourly job on the participant's computer fetches the ring's nightly and daily metrics for the last three days, so that later fetches correct values that were still incomplete. Until access is granted, and for exercise and time in daylight in any case, a daily iPhone automation reads Apple Health, to which the ring's app writes its data, and posts the values. The same hourly job fetches weather for the last complete days in the participant's time zone and reads the indoor air monitor's current values through the Alexa web service. Amazon offers no public interface for the monitor, so the connector uses the unofficial interface that home automation software uses, and it can stop working when Amazon changes its service. Indoor readings are sampled hourly while the computer is awake, so a day's mean usually covers waking hours rather than 24 hours; it is recorded only when at least 12 hours of that calendar day were sampled. Appendix A describes both connectors. Both are implemented and tested with synthetic responses shaped like the services' formats. The Amazon connector has also been checked against the live service during the pilot; the Ultrahuman connector will be checked when the Partner API token is granted, and until then the ring's values come through Apple Health.
...
1. tighter and wider priors: τ ~ HalfNormal(0.25) and HalfNormal(1) for the shared scale, and standard deviations of 0.25 and 1 for the confirmatory factors;
...
Phase 2, pilot (from 19 September 2026 to the study start): checks of the reminders, the automatic data flows, the backups and the evening log. Pilot reports test the software and never enter a model.
...
5. Open-Meteo receives the home location rounded to about one kilometer, to return the weather, and Apple's push service passes on the reminders, encrypted for the phone, so it learns only when they are sent;
...
Each factor has one source for the whole study; the alternative in brackets applies only if the primary source is not available at the study start. Dates are local calendar dates in the participant's time zone; the unit in the last column is the one for which odds ratios are reported.
...
| hrv_ms | Heart rate variability of the same night: Ultrahuman nightly average as reported by Ultrahuman (Apple Health: mean of overnight SDNN samples) | same day | 10 ms |
