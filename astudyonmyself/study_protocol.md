...
| Status | Finalized on 19 September 2026 as version 3.0 and corrected as version 3.1 the same day, before the study start; version 3.2, of the first study day, corrects descriptions of the software and changes nothing in the analysis plan (Revision History). The pilot ran on 19 September 2026 (Section 2.12). Confirmatory data collection starts on 20 September 2026, the day after finalization; that date is recorded in the software as the study start, and reports made before it never enter a model. |
...
| 3.2 | September 2026 | Corrected on 20 September 2026, the first study day, before any model was fitted, after a check of the running software against this protocol. The software changed in three places, and this protocol now describes them: (1) after a period without a successful fetch, the hourly job reaches back to the last successful one, not only three days, so that a few days with the computer off do not lose ring values (Section 2.4, Appendix A); (2) a reminder that could not be sent is tried again, and only a reminder the push service accepted counts as answered (Section 2.3.1); (3) the dashboard's descriptive summaries leave out the pilot reports and show the completeness of each factor (Sections 2.3.3 and 2.9.11). The outcomes, factors, sources, model, hypotheses and decision rules are unchanged. Not sent for review (Section 3.4). |
...
Weekly shares of Happy and Sad reports, shares by slot and by weekday, weekly adherence, the completeness of each factor, the share of reports that answer a reminder, and tag associations (the difference in the share of Happy reports with and without a tag, with Beta(1, 1) posteriors recomputed at every refit) are shown on the participant's private dashboard, together with a one-day forecast. They use the reports made from the study start, like the model, and are unadjusted; tag associations are same-moment, so a tag can be a cause of mood or a consequence of it. The forecast is a convenience for the participant, and its accuracy is not an objective of this study.
